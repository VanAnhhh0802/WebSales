...
export const useShoeStore = defineStore('shoe', () => {
  ...
  const getFeaturedShoes = computed(() => 
    shoes.value.filter(shoe => shoe.discount || shoe.rating && shoe.rating >= 4.5).slice(0, 4)
  );
  
  const getShoesByCategory = computed(() => (category: string) => 
    shoes.value.filter(shoe => shoe.category === category)
  );
  
  const getShoeById = computed(() => (id: number) => 
    shoes.value.find(shoe => shoe.id === id)
  );
